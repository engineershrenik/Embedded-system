 ...
 ***********************************************************************************************************************/
/*********************************************************************************************************************
 * File Name    : r_lpc_rx23w.c
 * Description  : Implementation of the LPC module for the RX23W
 **********************************************************************************************************************/
/*********************************************************************************************************************
...
static uint32_t hoco_frequency[4];     // Possible HOCO frequencies; Indices 1&2 are not used for the RX23W.
...
lpc_err_t lpc_operating_mode_set (lpc_operating_mode_t e_mode)
{
    ...
    if ((LPC_OP_HIGH_SPEED != e_mode) && (1 == SYSTEM.MEMWAIT.BIT.MEMWAIT))
    {
    ...
    switch (e_mode)
    {
        ...
        case LPC_OP_MIDDLE_SPEED:
            ...
            R_BSP_RegisterProtectDisable(BSP_REG_PROTECT_LPC_CGC_SWR);
            SYSTEM.OPCCR.BIT.OPCM = LPC_MEDIUM_SPD;  // Set to Medium Speed Mode
            R_BSP_RegisterProtectEnable(BSP_REG_PROTECT_LPC_CGC_SWR);
            /* WAIT_LOOP */
            while (LPC_TRANSITION_ONGOING == SYSTEM.OPCCR.BIT.OPCMTSF)
            {
...
static lpc_err_t lpc_freq_range_check (lpc_operating_mode_t e_mode)
{
    ...
    hoco_frequency[3] = 54000000; // 54 MHz
    ...
    {
        case LPC_OP_HIGH_SPEED:
            if (1 == SYSTEM.MEMWAIT.BIT.MEMWAIT)
            {
                if ((((((iclk_freq > LPC_MAX_ICK_HIGH_SPEED_FREQ_HZ2)) 
        ...
        case LPC_OP_MIDDLE_SPEED:
            ...
            /* Check if clock speeds are within limits */
            if (((((iclk_freq > LPC_MAX_ICK_MID_SPEED_FREQ_HZ)
                 || (fclk_freq > LPC_MAX_FCK_MID_SPEED_FREQ_HZ))
                 || (pclka_freq > LPC_MAX_PCKA_MID_SPEED_FREQ_HZ))
                 || (pclkb_freq > LPC_MAX_PCKB_MID_SPEED_FREQ_HZ))
                 || (pclkd_freq > LPC_MAX_PCKD_MID_SPEED_FREQ_HZ))
            {
                return LPC_ERR_CLOCK_EXCEEDED;
            }
        break;
        case LPC_OP_LOW_SPEED:
            ...
            /* Check if clock speeds are within limits */
            if (((((iclk_freq > LPC_MAX_ICK_LOW_SPEED_FREQ_HZ)
                 || (fclk_freq > LPC_MAX_FCK_LOW_SPEED_FREQ_HZ))
                 || (pclka_freq > LPC_MAX_PCKA_LOW_SPEED_FREQ_HZ))
                 || (pclkb_freq > LPC_MAX_PCKB_LOW_SPEED_FREQ_HZ))
                 || (pclkd_freq > LPC_MAX_PCKD_LOW_SPEED_FREQ_HZ))
            {
                return LPC_ERR_CLOCK_EXCEEDED;
            }
            if (((((LPC_CLOCK_ACTIVE == SYSTEM.LOCOCR.BYTE)     /* Check if LOCO is running */
                ...
                || (LPC_CLOCK_ACTIVE == SYSTEM.UPLLCR2.BYTE))   /* Check if UPLL is running */
            {
                return LPC_ERR_CLOCK_EXCEEDED;
            }
        break;
        ...
        if (LPC_MEDIUM_SPD == SYSTEM.OPCCR.BIT.OPCM) /* if return mode is medium speed mode */
        {
            ...
            switch(rstckcr_rstcksel)
            {
                ...
                case LPC_MAIN_OSC:
                    ...
                    /* When exiting sleep-mode to medium-speed mode with the main clock oscillator or LOCO selected,
                     * the system clock frequency does not exceed the limit */
                    if ((((((BSP_CFG_XTAL_HZ >> SYSTEM.SCKCR.BIT.ICK) > LPC_MAX_ICK_MID_SPEED_FREQ_HZ)
                         || ((BSP_CFG_XTAL_HZ >> SYSTEM.SCKCR.BIT.FCK) > LPC_MAX_FCK_MID_SPEED_FREQ_HZ))
                         || ((BSP_CFG_XTAL_HZ >> SYSTEM.SCKCR.BIT.PCKA) > LPC_MAX_PCKA_MID_SPEED_FREQ_HZ))
                         || ((BSP_CFG_XTAL_HZ >> SYSTEM.SCKCR.BIT.PCKB) > LPC_MAX_PCKB_MID_SPEED_FREQ_HZ))
                         || ((BSP_CFG_XTAL_HZ >> SYSTEM.SCKCR.BIT.PCKD) > LPC_MAX_PCKD_MID_SPEED_FREQ_HZ))
                    {
