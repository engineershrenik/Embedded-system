                      ...
                      const st_ble_servc_char_info_t **pp_char)
{
    for (uint8_t s = 0; s < gs_num_of_clients; s++)
    {
        ...
        {
            if ((gs_clients[s]->pp_chars[c]->p_attr_hdls[conn_idx].start_hdl <= attr_hdl) &&
        ...
        if (attr_hdl == p_attr->p_attr_hdls[conn_idx].start_hdl + 1)
        {
            if (BLE_SUCCESS == result)
            {
                ...
                else
                {
                    ...
                    p_client->cb(BLE_SERVC_MULTI_ATTR_EVENT(p_attr->char_idx, p_attr->inst_idx, BLE_SERVC_READ_RSP), BLE_SUCCESS, &evt_data);
                }
                ...
                if (attr_hdl == p_attr->pp_descs[d]->p_attr_hdls[conn_idx])
                {
                    if (BLE_SUCCESS == result)
                    {
                        ...
                        else
                        {
                            st_ble_servc_evt_data_t evt_data = {
                                .conn_hdl  = conn_hdl,
                                .param_len = p_attr->pp_descs[d]->app_size,
                                .p_param   = p_app_value,
                            };
                            p_client->cb(BLE_SERVC_MULTI_ATTR_EVENT(p_attr->pp_descs[d]->desc_idx, p_attr->inst_idx, BLE_SERVC_READ_RSP), BLE_SUCCESS, &evt_data);
                        }
        ...
        case BLE_GATTC_EVENT_HDL_VAL_NTF:
        {
            ...
            if ((NULL != p_attr) && (NULL != p_attr->decode))
            {
                ...
                else
                {
                    ...
                    p_client->cb(BLE_SERVC_MULTI_ATTR_EVENT(p_attr->char_idx, p_attr->inst_idx, BLE_SERVC_HDL_VAL_NTF), BLE_SUCCESS, &evt_data);
                }
        ...
        case BLE_GATTC_EVENT_HDL_VAL_IND:
        {
            ...
            if ((NULL != p_attr) && (NULL != p_attr->decode))
            {
                ...
                else
                {
                    ...
                    p_client->cb(BLE_SERVC_MULTI_ATTR_EVENT(p_attr->char_idx, p_attr->inst_idx, BLE_SERVC_HDL_VAL_IND), BLE_SUCCESS, &evt_data);
                }
            ...
            for (uint8_t c = 0; c < p_info->num_of_chars; c++)
            {
                    ...
                    (BLE_GATT_INVALID_ATTR_HDL_VAL == p_info->pp_chars[c]->p_attr_hdls[conn_idx].start_hdl))
                {
                    ...
                    for (uint8_t d = 0; d < p_char_param->num_of_descs; d++)
                    {
                        ...
                        {
                            if ((p_info->pp_chars[c]->pp_descs[dd]->uuid_16 == p_char_param->descs[d].value.desc_16.uuid_16) &&
