...
*         : 20.11.2020 1.03     Changed the sub clock initialization process when the battery backup function is 
*                               enabled.
*         : 29.01.2021 1.04     Fixed the initialization settings of sub-clock for Technical Update Information
*                               (TN-RX*-A0237B).
***********************************************************************************************************************/
...
/* Clock source = HOCO(32MHz) ICLK = PCLKA = PCLKB = PCLKD = FCLK = 32MHz / 1 = 32MHz */
#if ((BSP_CFG_MCU_VCC_MV >= 2700) && (BSP_CFG_MCU_VCC_MV <= 3600))
#define BSP_PRV_HOCO_SCKCR     (0x00000000)

/* Clock source = HOCO(32MHz) ICLK = PCLKA = PCLKB = PCLKD = FCLK = 32MHz / 2 = 16MHz */
#elif ((BSP_CFG_MCU_VCC_MV >= 2400) && (BSP_CFG_MCU_VCC_MV < 2700))
#define BSP_PRV_HOCO_SCKCR     (0x11011111)

/* Clock source = HOCO(32MHz) ICLK = PCLKA = PCLKB = PCLKD = FCLK = 32MHz / 4 = 8MHz */
#elif ((BSP_CFG_MCU_VCC_MV >= 1800) && (BSP_CFG_MCU_VCC_MV < 2400))
...
static void clock_change_hoco(void);
#else
...
#endif
{
  ...
  #endif
#else /* BSP_CFG_ICK_DIV <= BSP_CFG_PCKB_DIV */
...
#if BSP_CFG_CLKOUT_RF_MAIN == 1
/***********************************************************************************************************************
* Function name: clock_change_hoco
* Description  : 
* Arguments    : none
* Return value : none
***********************************************************************************************************************/
static void clock_change_hoco(void)
{
    ...
    /* Hoco clock division setting */
    SYSTEM.SCKCR.LONG = BSP_PRV_HOCO_SCKCR;

    if(dummy_read ==  SYSTEM.SCKCR.LONG)
    {
        R_BSP_NOP();
    }
    ...
    /* Dummy read and compare. cf."5. I/O Registers", "(2) Notes on writing to I/O registers" in User's manual.
       ...
       pipeline architecture so the next instruction could be executed before the previous write had finished.
    */
    if(dummy_read ==  SYSTEM.SCKCR3.WORD)
    {
        R_BSP_NOP();
    }
...
} /* End of function clock_change_hoco() */
#endif /* BSP_CFG_CLKOUT_RF_MAIN == 1 */
